			     ...
			     int * dir )
{
  ...
  do {
      index = rand() % (s->params->n_dims);
  } while (!options[index]);
  
  if (options[index] == 1) {
      dest_id = s->neighbour_plus_lpID[index];
      *dir = 1;
  }
  else {
      dest_id = s->neighbour_minus_lpID[index];
      *dir = 0;
  }
		    ...
		    tw_lp * lp )
{
  ...
  if( lp->gid == msg->dest_lp )
    {   
	    ...
	    printf("$ sending server %d talked to receiving server %d in %d hops\n",
		   codes_mapping_get_lp_relative_id(msg->sender_svr,0,1), 
		   codes_mapping_get_lp_relative_id(msg->dest_lp,0,1), msg->my_N_hop);
	    // Trigger an event on receiving server
	    if(msg->remote_event_size_bytes)
	    {
